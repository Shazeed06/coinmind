  ...
  return (
    <footer className="border-t border-border bg-bg-alt">
      <div className="container-main py-24">
          ...
          {footerNav.map((col, i) => (
            <div key={col.title}>
              <button
                ...
                className="lg:cursor-default flex items-center justify-between w-full text-xs font-semibold uppercase tracking-wider text-text-muted lg:pointer-events-none"
              >
                {col.title}
                <ChevronDown className={`h-3.5 w-3.5 lg:hidden transition-transform ${openCol === i ? "rotate-180" : ""}`} />
              </button>
              <ul className={`mt-4 space-y-3 ${openCol === i ? "block" : "hidden lg:block"}`}>
                {col.items.map((l) => (
                  <li key={l.href}>
        ...
        <div className="mt-16 pt-8 border-t border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs text-text-muted">
